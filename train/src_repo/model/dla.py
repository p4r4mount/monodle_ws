...
class Tree(nn.Module):
    def __init__(self, levels, block, in_channels, out_channels, stride=1,
        ...
        if stride > 1:
            ...
            # self.downsample = nn.Conv2d(in_channels,out_channels,3,stride,padding=1)
...
class DLA(nn.Module):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        # model_pth = '/home/song/Proj/Rope3D/monodle_models/wusongzuiaidemoxing.pth'
        # model_weights = torch.load(model_pth)
        num_classes = len(model_weights[list(model_weights.keys())[-1]])
        self.fc = nn.Conv2d(
            self.channels[-1], num_classes,
            kernel_size=1, stride=1, padding=0, bias=True)
        self.load_state_dict(model_weights["model_state"])
        self.fc = fc
...
def dla34(pretrained=False, **kwargs):  # DLA-34
    ...
    # model.load_state_dict({k.replace('backbone.', ''): v for k, v in torch.load(model_pth)['model_state'].items()})
    if pretrained:
